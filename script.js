...
const result = text.replace(/;/g, ";\n"); //вопрос: почему в подсказке пишется с двумя слешами? у меня сработало только с одним слешом, с двумя нет

console.log(`2. Текст с переносами строк: ${result}`);

const substring = result.replace(/\s/g, "");
console.log(`3. Текст с удалёнными пробелами: ${substring}`); // тут тоже самое, в подсказке с двумя слешами, но с двумя не работает, только с одним, почему так?

const newText = substring.slice(28, 49); //я правильно пишу 49? раз просят не включительно до 50-го символа
...
); //почему у меня тут перенослись скобки? подскажи пожалуйста :)
